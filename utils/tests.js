...
const {Addresses,TxsTokens, Transactions, Prices, Blocks, Tokens, sequelize } = require('../models');
...
async function getAllTokenTransfers(abi, address) {
	...
	let allTransfers = await tokenName.getPastEvents('Transfer', {
		fromBlock: 800000,
		toBlock: 'latest'
	});
	for(let tranfer of allTransfers) {
		console.log(tranfer)
	}
